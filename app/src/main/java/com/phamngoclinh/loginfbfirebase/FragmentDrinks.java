...
public class FragmentDrinks extends Fragment{
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        layoutHome = inflater.inflate(R.layout.fragment_drinks,container,false);
        mData = FirebaseDatabase.getInstance().getReference();
        recyclerDr();
        return layoutHome;
    }

    public void recyclerDr(){
        recyclerviewBook = layoutHome.findViewById(R.id.recycleOrderDr);
        linearLayoutManager = new LinearLayoutManager(getActivity(), LinearLayoutManager.VERTICAL, false);

        bookArrayList = new ArrayList<>();
        mData.child("Food").child("Drinks").addChildEventListener(new ChildEventListener() {
            @Override
